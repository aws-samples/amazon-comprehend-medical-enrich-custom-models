...
15.	(**Optional**) The following step is optional, but illustrates how you can dig deeper into the Comprehend Custom classifier performance. To do this, we will be running the Comprehend Custom classifier on a separate testing set, and plotting the results.
	...
	4.	Click `Create job`
	5.	After a few minutes, the job will finish. Download the results, unzip the *tar.gz* file, and run the code provided [here]???. The output will look like: ![Image](doc/image_15.png)
...
Next, download and deploy [this](https://amazon.awsapps.com/workdocs/index.html#/document/d3c3ab90dcd513d92c037544ddd587841ee31e2645c3396596e7a4d32c8cd84d) CloudFormation (CFN) Template, and deploy it via the [CloudFormation console](https://console.aws.amazon.com/cloudformation/home). The CFN template is available in this repository in the file `cfn_lambda_cm_comprehend_custom.yaml` As before, this CFN is configured to run out-of-the-box in the `us-east-1` region. If you are running it in any other region, you will have to modify some of these steps.
...
1. Download the document from [here](https://amazon.awsapps.com/workdocs/index.html#/document/2ae3b7a77111b16b4a75e20fd82cef42760bec1bc661495c08546b81cc512c42). You can also find this file in the `files` directory. It contains just one sentence:
...
3. After waiting about 30 seconds (to give the Lambda function time to process the file), check the contents of ResultsBucket; it should contain a file that contains the results of the Lambda function in JSON format. The raw results can be found [here](https://amazon.awsapps.com/workdocs/index.html#/document/fdeac24a63e0e416d531ac2d99c3974b71352fbc5f5f09248c87f536be667354), but for clarity we also show them below. We have highlighted in peach the additional parts of the JSON object that are not included in regular Comprehend Medical.
